...
$(document).ready(() => {
    ...
    const resultsBuilder = () => {
        ...
        console.log('boolean', )
         ...
         if(Boolean(answers[2].value)) $('.sm-results--1b--wrapper').find('.results--text').text(answers[2].value)
    ...
    const specialSliderHandler = (amount) => {
        ...
        const handleDrag = function (ui, $button, isButton1, $button1, $button2, $sliderBlock) {
            ...
            if (isButton1) {
                ...
                else if (dividerIndex === 3) $('.sm--graph--block--1').css('width', '73%')
                else if (dividerIndex === 4) $('.sm--graph--block--1').css('width', '100%')
    ...
    $('.sm--button').on('click', (e) => {
        ...
        if (clickedButton.hasClass('next') || clickedButton.hasClass('sm--unsure') || clickedButton.hasClass('yes') || clickedButton.hasClass('no')) {
            ...
            if (state === 3) {
                ...
                answers[8].value = $('#Full-name').val();
                answers[9].value = $('#Email-address').val();
                answers[10].value = $('#Phone-number').val();
    ...
    function parseInsurancePremium() {
        var value = $('#annual-insurance-premium').val();
        var parsedValue = value.replace(/[^0-9]/g, '');  // Remove any character that is not a digit
    ...
    function initializeSlider() {
        $(document).ready(function () {
            $('.sm-slider--progress--button').each(function (index, button) {
                var $sliderBlock = $(button).closest('.sm-slider--block');
                var sliderText = $(button).closest('.slider--wrapper').find('.sm-input--slider--text').text().replace(/\s+/g, '-');
                ...
                $(button).draggable({
                    ...
                    drag: function (event, ui) {
                        ...
                        answers[2].value = percentage + '%';
                        $progressBar.css('width', `${$(button).position().left}px`);
                    }
                });
            });

            $('.sm-slider--progress--button').on('mousedown', (e) => {
                if (!$(e.target).hasClass('active')) $(e.target).addClass('active');
                if (!$(e.target).find('.sm-slider--percentage--percentage--text').hasClass('active')) $(e.target).find('.sm-slider--percentage--percentage--text').addClass('active');
                if (!$(e.target).siblings('.sm-slider--progress').hasClass('active')) $(e.target).siblings('.sm-slider--progress').addClass('active');
            });
        });
    }
    ...
    function resultsNavigator() {
        $('.s-label--results').on('click', (e) => {
            ...
            } else if (clickedButton.is('#state--1')) {
                state = 1;
                $('.back').trigger('click');
